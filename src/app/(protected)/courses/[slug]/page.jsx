...
const BulletListBlock = ({ items }) => (
  <ul className="space-y-3 mb-6">
    {items.map((item, idx) => (
      <li key={idx} className="flex gap-3 bg-white p-3 rounded-lg border border-gray-100 shadow-sm">
        <div className="mt-1 min-w-4 w-4 h-4 rounded-full bg-blue-100 flex items-center justify-center shrink-0">
          <div className="w-2 h-2 rounded-full bg-blue-600"></div>
        </div>
        <div>
          <span className="font-semibold text-gray-800 block mb-1">{item.title}</span>
...
const CalloutBlock = ({ style, title, content }) => {
  ...
  return (
    <div className={`p-4 rounded-xl border flex gap-3 mb-6 ${styles[style] || styles.info}`}>
...
const BlockRenderer = ({ block }) => {
  switch (block.type) {
    ...
    case "bullet-list": return <BulletListBlock {...block} />;
...
export default function Course() {
  ...
  if (!course) {
    ...
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-gray-50 p-4">
        ...
        <p className="text-gray-600 mt-2">The course you are looking for does not exist or could not be loaded.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen bg-white overflow-hidden font-sans">
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  <p className={`text-xs mt-1 line-clamp-1 ${isActive ? 'text-blue-100' : 'text-gray-500'}`}>
                    {chapter.description}
                  </p>
                </div>
                {isActive && <ChevronRight className="w-4 h-4 ml-auto self-center opacity-80" />}
              </button>
            );
          })}
        </div>
      ...
      {/* Main Content Area */}
      <main id="main-content" className="flex-1 overflow-y-auto bg-white relative scroll-smooth">
        {loadingChapter ? (
          ...
          <div className="max-w-4xl mx-auto px-6 py-12 lg:px-12">
            ...
            <div className="mb-12">
              <div className="flex items-center gap-2 text-blue-600 font-bold text-sm tracking-wide uppercase mb-4">
                <span className="bg-blue-50 px-3 py-1 rounded-full">Chapter {chapterContent.chapterNumber}</span>
                <span>•</span>
