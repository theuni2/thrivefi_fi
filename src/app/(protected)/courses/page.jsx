...
export default function CoursesPage() {
  ...
  const handleEnroll = async (e, course) => {
    ...
    try {
      ...
      setCourses((prev) =>
        prev.map((c) =>
          c.slug === course.slug ? { ...c, isEnrolled: true } : c
        )
      );
      ...
      if (res.ok) {
        ...
        setCourses((prev) =>
          prev.map((c) =>
            c.slug === course.slug ? { ...c, isEnrolled: false } : c
          )
        );
      }
    } catch (err) {
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 relative z-10 text-center">
              <div className="max-w-3xl mx-auto">
                ...
                <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-6 leading-tight">
                  ...
                  <span className="text-indigo-400">Finance & AI</span>
                </h1>
                <p className="text-lg text-indigo-200 mb-8 leading-relaxed">
                  Explore our curated curriculum designed to bridge the gap
                  between traditional banking and cutting-edge technology.
                </p>
              </div>
            </div>
          </div>
